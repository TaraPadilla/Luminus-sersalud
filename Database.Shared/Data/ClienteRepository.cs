...
namespace Database.Shared.Data
{
    public class ClienteRepository : ICliente
    {
        ...
        public List<object> GetListCombinada()
        {
            var clientes = _context.Clientes.Where(x => x.Eliminado == false).ToList();

            var pacientes = _context.Pacientes.Where(x => x.Eliminado == false).ToList();

            // Combina las listas de clientes y pacientes en una lista de objetos anónimos
            var listaCombinada = clientes.Cast<object>().Concat(pacientes.Cast<object>()).ToList();
