...
//                 && cuentaValorRedondeado > 0m
//                 && valorTotalCalculadoRedondeado > cuentaValorRedondeado)
//             {
//                 Console.WriteLine(
//                     $"[SYNC] Diferencia detectada (incremento). " +
...
//             }
//             else if (cuentaValorRedondeado != valorTotalCalculadoRedondeado)
//             {
//                 Console.WriteLine(
//                     $"[SYNC] Diferencia detectada pero NO se actualiza. " +
//                     $"Cuenta.Valor={cuentaValorRedondeado:0.00}, Calculado={valorTotalCalculadoRedondeado:0.00}. " +
//                     $"Probablemente ya hay pagos/descuentos aplicados."
//                 );
//             }
...
//                 var totalCuenta = Math.Round(Convert.ToDecimal(cuenta.Valor ?? 0m), 2); // Saldo pendiente actual en BD
//                 var pagadoAnterior = Math.Round(Convert.ToDecimal(cuenta.ValorPagado ?? 0m), 2);

//                 var porcentajeDescuento = Math.Round(Math.Max(Convert.ToDecimal(model.PorcentajeDescuento), 0m), 2);

//                 decimal descuentoAplicado;
//                 if (porcentajeDescuento > 0m)
//                 {
//                     descuentoAplicado = Math.Round(totalCuenta * porcentajeDescuento / 100m, 2, MidpointRounding.AwayFromZero);
//                 }
//                 else
//                 {
//                     descuentoAplicado = Math.Round(Math.Max(Convert.ToDecimal(model.Descuento), 0m), 2, MidpointRounding.AwayFromZero);
//                 }

//                 if (descuentoAplicado > totalCuenta)
//                     descuentoAplicado = totalCuenta;

//                 var saldoPendienteReal = Math.Round(totalCuenta - descuentoAplicado, 2);

//                 Console.WriteLine($"[DEBUG PAGO] TotalCuenta (neto BD): {totalCuenta}, PagadoAnterior: {pagadoAnterior}");
//                 Console.WriteLine($"[DESCUENTO]  Descuento: {descuentoAplicado} ({porcentajeDescuento}%), SaldoPendienteConDescuento: {saldoPendienteReal}");
...
//                             {
//                                 Exitoso = false,
//                                 Mensaje = $"El monto a pagar excede el saldo pendiente con descuento. " +
//                                          $"Saldo pendiente: Q {saldoPendienteReal:0.00}, " +
//                                          $"Excede por: Q {excedente:0.00}. Por favor verificar monto."
//                             });
//                         }
...
//                 cuenta.Valor = Math.Max(Math.Round((cuenta.Valor ?? 0m) - totalPagoNuevo - descuentoAplicado, 2, MidpointRounding.AwayFromZero), 0m);
//                 cuenta.FechaPagoRealizado = fechaHora;
...
//                 Console.WriteLine($"DescuentoAplicado: {descuentoAplicado} ({porcentajeDescuento}%)");
//                 Console.WriteLine($"SaldoPendienteConDescuento: {saldoPendienteReal}");
//                 Console.WriteLine($"TotalCuenta ACTUALIZADO (después de pago+descuento): {saldoRestante}");
...
//                 Console.WriteLine($"TotalFactura (para FEL): {totalFactura}");
...
//                 if (cuentaCerrada)
//                 {
//                     mensajeUsuario = descuentoAplicado > 0m
//                         ? $"¡Pago exitoso! La cuenta ha sido cancelada con un descuento de Q {descuentoAplicado:0.00}."
//                         : "¡Pago exitoso! La cuenta ha sido cancelada en su totalidad.";
//                 }
//                 else
//                 {
