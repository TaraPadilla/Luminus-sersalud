function FarmaciaViewModel() {
    ...
    self.obtenerCategoriasHabitacion = function () {
        $.ajax({
            url: "/SolicitudMedicamento/GetAllCategoriasHabitacion",
            type: "GET",
    ...
    self.obtenerSolicitudes = function () {
        ...
        let url = "/SolicitudMedicamento/GetAll";
        let params = [];
    ...
    //     $.ajax({
    //         url: `/SolicitudMedicamento/MarkAsDispensed/${id}`,
    //         type: 'PUT',
    //         contentType: 'application/json',
...
self.despacharSolicitud = function (id) {
    $.ajax({
        ...
        success: function (response) {
            if (response.exitoso) {   // ← minúscula
                self.obtenerSolicitudes();
                self.actualizarEstadoSolicitud(id, "Despachado");
            } else {
                alert(`❌ Error al despachar: ${response.mensaje}`); // ← minúscula
            }
        },
        error: function (xhr, status, error) {
            let msg = xhr.responseJSON ? (xhr.responseJSON.mensaje || error) : error;
