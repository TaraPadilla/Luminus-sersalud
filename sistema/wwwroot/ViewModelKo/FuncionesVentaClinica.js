    ...
    data.forEach(function (item) {
        ...
        if (item.tipoProductoId === 1) {
            ...
            var htmlTags =
                ...
                `<label class="badge badge-warning">${item.unidadVenta}</label>` +
                '</div>' +
                '<br>' +
                '<br>' +
...
function agregarATabla(data) {
    ...
    var htmlTags = '<tr style="background-color:rgba(255,211,105,0.5);"> <td> ' + data.codigoReferencia + '</td>' +
        ...
        '<option value="' + data.precio_6.toFixed(2) + '"> Precio vip - Q' + data.precio_6.toFixed(2) + '</option>' +
...
function GuardarVentaClinica() {
    ...
    $.each($("#tableventa tbody tr"), function () {
        detalleVenta.push({
            ...
            "UsuarioAutorizaModificacion": this.querySelectorAll("td.usuarioAutoriza")[0].querySelectorAll("input")[0].value
    ...
    if (saldo == 0) {
        ...
        $.ajax({
            ...
            url: '/Venta/GuardarVentaClinica',
            traditional: true,
            success: function (data) {
                window.open('/CrearPDF/ReciboVentaPdf/' + data, '_blank');
                window.location.href = '/Venta/NuevaVentaClinica/';
            },
            error: function (data) {
